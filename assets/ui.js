...
function renderTable({tableEl,rows,columns,formatters={},onRowClick}){const thead=tableEl.querySelector("thead"); const tbody=tableEl.querySelector("tbody");
thead.innerHTML=`<tr>${columns.map(c=>`<th>${escapeHtml(c.label)}</th>`).join("")}</tr>`;
tbody.innerHTML=rows.map((row,idx)=>`<tr data-row-index="${idx}" style="${onRowClick?'cursor:pointer':''}">${columns.map(c=>{let value=row?.[c.key]; if(formatters[c.key]) value=formatters[c.key](value,row);
if(c.badge){const cls=badgeClass(value); return `<td><span class="badge text-bg-${cls}">${escapeHtml(value)}</span></td>`;}
const text=value??""; const mono=c.mono?"text-mono":""; return `<td class="text-truncate ${mono}" style="max-width:380px" title="${escapeHtml(text)}">${escapeHtml(text)}</td>`;}).join("")}</tr>`).join("");
if(onRowClick){tbody.querySelectorAll("tr").forEach(tr=>tr.addEventListener("click",()=>{const i=Number(tr.dataset.rowIndex); onRowClick(rows[i]);}));}}
